...
import { Types, Options, CanvasBgColor } from './options';
(() => {
    let dstype = 'graph';
    ...
    function visualize() {
        ...
        if (Parser.validate_input(input)) {
            let parsed_input = Parser.parse_input(input, dstype, dsinputtype, InputOptions);
    ...
    function init() {
        textarea = document.getElementById('dataset-textarea');
        ...
        weighted_switch = document.getElementById('weighted_switch');
        weighted_switch.addEventListener('click', (event) => {
            ...
            if (InputOptions.graph.weighted) {
                textarea.setAttribute('placeholder', InputTypes.graph.adjacency_list.placeholder2);
            }
            else {
                textarea.setAttribute('placeholder', InputTypes.graph.adjacency_list.placeholder);
            }
        });
        document
